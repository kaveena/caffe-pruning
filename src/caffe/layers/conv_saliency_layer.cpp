...
//  Caffe::set_derivative_compute(true); //if any Convolution Saliency layer exists then need ddiff computation
  ...
  this->saliency_channel_ = this->layer_param_.convolution_saliency_param().channel_saliency();
...
void ConvolutionSaliencyLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  if (this->output_channel_saliency_compute_) {
    if (this->layer_param_.convolution_saliency_param().saliency() == caffe::ConvolutionSaliencyParameter::ALL) {
      output_saliencies_channel_.Reshape({(int)(caffe::ConvolutionSaliencyParameter::ALL), this->conv_out_channels_});
    }
    else {
      output_saliencies_channel_.Reshape({1, this->conv_out_channels_});
    }
    output_saliencies_points_.Reshape(top[0]->shape()); //shape nmhw
    output_saliencies_filter_.Reshape({this->num_, this->conv_out_channels_}); //shape nm
  }
  if (this->input_channel_saliency_compute_) {
    if (this->layer_param_.convolution_saliency_param().saliency() == caffe::ConvolutionSaliencyParameter::ALL) {
      input_saliencies_channel_.Reshape({(int)(caffe::ConvolutionSaliencyParameter::ALL), this->conv_in_channels_});
    }
    else {
      input_saliencies_channel_.Reshape({1, this->conv_in_channels_});
    }
    input_saliencies_points_.Reshape(bottom[0]->shape()); //shape nchw
    input_saliencies_filter_.Reshape({this->num_, this->conv_in_channels_}); //shape nc
  }
}
  ...
  for (int i = 0; i < top.size(); ++i) {
    ...
    if (this->saliency_input_ == caffe::ConvolutionSaliencyParameter::WEIGHT) {
      ...
      switch (this->saliency_) {
        ...
        case (8): {
          compute_fisher_weights_cpu(&weights_n_masked_, &bias_n_masked_, output_channel_saliency_data, input_channel_saliency_data);
          compute_taylor_weights_cpu(&weights_n_masked_, &bias_n_masked_, output_channel_saliency_data + (this->conv_out_channels_), input_channel_saliency_data + (this->conv_in_channels_));
          compute_hessian_diag_weights_cpu(&weights_n_masked_, &bias_n_masked_, output_channel_saliency_data + (2 * this->conv_out_channels_), input_channel_saliency_data + (2 * this->conv_in_channels_));
          compute_hessian_diag_approx2_weights_cpu(&weights_n_masked_, &bias_n_masked_, output_channel_saliency_data + (3 * this->conv_out_channels_), input_channel_saliency_data + (3 * this->conv_in_channels_));
          compute_taylor_2nd_weights_cpu(&weights_n_masked_, &bias_n_masked_, output_channel_saliency_data + (4 * this->conv_out_channels_), input_channel_saliency_data + (4 * this->conv_in_channels_));
          compute_taylor_2nd_approx2_weights_cpu(&weights_n_masked_, &bias_n_masked_, output_channel_saliency_data + (5 * this->conv_out_channels_), input_channel_saliency_data + (5 * this->conv_in_channels_));
          compute_weight_avg_weights_cpu(&weights_n_masked_, &bias_n_masked_, output_channel_saliency_data + (6 * this->conv_out_channels_), input_channel_saliency_data + (6 * this->conv_in_channels_));
          compute_diff_avg_weights_cpu(&weights_n_masked_, &bias_n_masked_, output_channel_saliency_data + (7 * this->conv_out_channels_), input_channel_saliency_data + (7 * this->conv_in_channels_));
        } break;
      ...
      switch (this->saliency_) {
        ...
        case (8): {
          compute_fisher_cpu(top_data, top_diff, channel_saliency_data);
          compute_taylor_cpu(top_data, top_diff, channel_saliency_data + this->conv_out_channels_);
          compute_hessian_diag_cpu(top_data, top_diff, top_ddiff, channel_saliency_data + (2*this->conv_out_channels_));
          compute_hessian_diag_approx2_cpu(top_data, top_diff, channel_saliency_data + (3*this->conv_out_channels_));
          compute_taylor_2nd_cpu(top_data, top_diff, top_ddiff, channel_saliency_data + (4*this->conv_out_channels_));
          compute_taylor_2nd_approx2_cpu(top_data, top_diff, channel_saliency_data + (5*this->conv_out_channels_));
          compute_weight_avg_cpu(top_data, channel_saliency_data + (6*this->conv_out_channels_));
          compute_diff_avg_cpu(top_diff, channel_saliency_data + (7*this->conv_out_channels_));
        } break;
...
void ConvolutionSaliencyLayer<Dtype>::compute_norm_and_batch_avg_cpu(int count, Dtype * output_saliency_data, Dtype * saliency_data, Dtype * bias_saliency_data) {
  ...
  switch (this->saliency_norm_) {
    case (caffe::ConvolutionSaliencyParameter::L1): {
      caffe_abs(this->num_ * this->conv_out_channels_ * count, output_saliency_data, output_saliency_data);
      if (this->saliency_bias_ && this->bias_term_ && bias_saliency_data != NULL){
        caffe_abs(this->num_ * this->conv_out_channels_, bias_saliency_data, bias_saliency_data);
      }
    } break;
    
    case (caffe::ConvolutionSaliencyParameter::L2): {
      caffe_powx(this->num_ * this->conv_out_channels_ * count, output_saliency_data, (Dtype) 2, output_saliency_data);
      if (this->saliency_bias_ && this->bias_term_ && bias_saliency_data != NULL){
        caffe_powx(this->num_ * this->conv_out_channels_, bias_saliency_data, (Dtype) 2, bias_saliency_data);
      }
    } break;
  ...
  caffe_sum(this->num_ * this->conv_out_channels_, count, output_saliency_data, filter_saliency_data); //sum hxw
  if (this->saliency_bias_ && this->bias_term_ && bias_saliency_data != NULL){
    caffe_add(this->num_ * this->conv_out_channels_, bias_saliency_data, filter_saliency_data, filter_saliency_data);
  }
  caffe_strided_sum(this->conv_out_channels_, this->num_, filter_saliency_data, saliency_data);
  caffe_scal(this->conv_out_channels_, 1 / (Dtype)(this->num_), saliency_data);

}

template void ConvolutionSaliencyLayer<float>::compute_norm_and_batch_avg_cpu(int count, float * output_saliency_data, float * saliency_data, float * bias_saliency_data);
template void ConvolutionSaliencyLayer<double>::compute_norm_and_batch_avg_cpu(int count, double * output_saliency_data, double * saliency_data, double * bias_saliency_data);
