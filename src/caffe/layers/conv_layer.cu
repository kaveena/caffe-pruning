...
void ConvolutionLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int i = 0; i < bottom.size(); ++i) {
    ...
    if (this->mask_term_ && this->quantize_term_) {
      caffe_gpu_and(bottom[i]->count(), this->quantization_mask, bottom_data, bottom_data);
    }
    ...
    if (this->saliency_input_ == caffe::ConvolutionSaliencyParameter::WEIGHT) {
      ...
      switch (this->saliency_) {
        ...
        case (8): {
          ...
          compute_weight_avg_weights_gpu(&weights_n_masked_, &bias_n_masked_, channel_saliency_data + (5*this->num_output_));
          compute_diff_avg_weights_gpu(&weights_n_masked_, &bias_n_masked_, channel_saliency_data + (6*this->num_output_));
        } break;
      ...
      switch (this->saliency_) {
        ...
        case (8): {
          ...
          compute_weight_avg_gpu(top_data, channel_saliency_data + (6*this->num_output_));
          compute_diff_avg_gpu(top_diff, channel_saliency_data + (7*this->num_output_));
        } break;
...
template <typename Dtype>
void ConvolutionLayer<Dtype>::compute_norm_and_batch_avg_gpu(int count, Dtype * output_saliency_data, Dtype * saliency_data, Dtype * bias_saliency_data) {
...
template void ConvolutionLayer<float>::compute_norm_and_batch_avg_gpu(int count, float * output_saliency_data, float * saliency_data, float * bias_saliency_data);
template void ConvolutionLayer<double>::compute_norm_and_batch_avg_gpu(int count, double * output_saliency_data, double * saliency_data, double * bias_saliency_data);
