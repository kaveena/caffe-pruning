...
void ConvolutionSaliencyLayer<Dtype>::compute_weight_avg_gpu(const Dtype *  act_data, Dtype * saliency_info) {
...
template <typename Dtype>
void ConvolutionSaliencyLayer<Dtype>::compute_weight_avg_weights_gpu(Blob<Dtype> * weights_n, Blob<Dtype> * bias_n, Dtype * saliency_info) {
  ...
  caffe_gpu_sum(this->conv_out_channels_, this->blobs_[0]->count(1,4), points_saliency_data, saliency_info); //sum hxw
  if (this->saliency_bias_ && this->bias_term_ && bias_saliency_data != NULL){
    caffe_gpu_add(this->conv_out_channels_, bias_saliency_data, saliency_info, saliency_info);
  }
}

template <typename Dtype>
void ConvolutionSaliencyLayer<Dtype>::compute_diff_avg_gpu(const Dtype *  act_diff, Dtype * saliency_info) {
  Dtype* output_saliency_data = output_saliencies_points_.mutable_gpu_data();    
...
template <typename Dtype>
void ConvolutionSaliencyLayer<Dtype>::compute_diff_avg_weights_gpu(Blob<Dtype> * weights_n, Blob<Dtype> * bias_n, Dtype * saliency_info) {
...
template void ConvolutionSaliencyLayer<float>::compute_weight_avg_gpu(const float *  act_data, float * saliency_info);
template void ConvolutionSaliencyLayer<double>::compute_weight_avg_gpu(const double *  act_data, double * saliency_info);

template void ConvolutionSaliencyLayer<float>::compute_weight_avg_weights_gpu(Blob<float> * weights_n, Blob<float> * bias_n, float * saliency_info);
template void ConvolutionSaliencyLayer<double>::compute_weight_avg_weights_gpu(Blob<double> * weights_n, Blob<double> * bias_n, double * saliency_info);

template void ConvolutionSaliencyLayer<float>::compute_diff_avg_gpu(const float *  act_diff, float * saliency_info);
template void ConvolutionSaliencyLayer<double>::compute_diff_avg_gpu(const double *  act_diff, double * saliency_info);

template void ConvolutionSaliencyLayer<float>::compute_diff_avg_weights_gpu(Blob<float> * weights_n, Blob<float> * bias_n, float * saliency_info);
template void ConvolutionSaliencyLayer<double>::compute_diff_avg_weights_gpu(Blob<double> * weights_n, Blob<double> * bias_n, double * saliency_info);
