...
void ConvolutionLayer<Dtype>::compute_hessian_diag_approx2_cpu(const Dtype *  act_data, const Dtype *  act_diff, Dtype * hessian_diag) {
...
template <typename Dtype>
void ConvolutionLayer<Dtype>::compute_hessian_diag_approx2_weights_cpu(Blob<Dtype> * weights_n, Blob<Dtype> * bias_n, Dtype * hessian_diag) {
...
template void ConvolutionLayer<float>::compute_hessian_diag_approx2_cpu(const float *  act_data, const float *  act_diff, float * hessian_diag);
template void ConvolutionLayer<double>::compute_hessian_diag_approx2_cpu(const double *  act_data, const double *  act_diff, double * hessian_diag);

template void ConvolutionLayer<float>::compute_hessian_diag_approx2_weights_cpu(Blob<float> * weights_n, Blob<float> * bias_n, float * hessian_diag);
template void ConvolutionLayer<double>::compute_hessian_diag_approx2_weights_cpu(Blob<double> * weights_n, Blob<double> * bias_n, double * hessian_diag);
}  // namespace caffe
